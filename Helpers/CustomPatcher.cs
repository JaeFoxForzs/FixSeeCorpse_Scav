...
namespace FixSeeCorpse.Helpers
{
    ...
    internal class CustomPatcher
    {
        internal static void PatchByName(Harmony harmony, Type targetClassType)
        {
            ...
            var corpsePatches = patches.Where(p => !p.Name.Contains("Krokosha")).ToList();
            
            PatchMethods(harmony, targetClassType, corpsePatches);
        }

        internal static void PatchMultiplayerClass(Harmony harmony, Type targetClassType)
        {
            List<MethodInfo> patches = typeof(MyPatches).GetMethods(BindingFlags.Static | BindingFlags.Public).ToList();
            
            var multiplayerPatches = patches.Where(p => p.Name.Contains("Krokosha")).ToList();
            
            PatchMethods(harmony, targetClassType, multiplayerPatches);
        }

        private static void PatchMethods(Harmony harmony, Type targetClassType, List<MethodInfo> patches)
        {
